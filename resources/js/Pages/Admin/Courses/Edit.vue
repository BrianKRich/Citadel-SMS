...
const form = useForm({
    ...
    grading_type:        props.course.grading_type ?? 'credit_system',
...
function submit() {
    form.patch(route('admin.courses.update', props.course.id));
}
</script>

<template>
    <Head :title="`Edit: ${course.course_code} — ${course.name}`" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Edit Course</h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
                <Breadcrumb :items="[
                    ...
                    { label: course.name, href: route('admin.courses.show', course.id) },
                    { label: 'Edit' },
                ]" />
                ...
                <Card>
                    <PageHeader :title="`Edit: ${course.course_code} — ${course.name}`" />
