...
const form = useForm({
    course_id: cls.value?.course_id ?? '',
    employee_id: cls.value?.employee_id ?? '',
    academic_year_id: cls.value?.academic_year_id ?? '',
    term_id: cls.value?.term_id ?? '',
    section_name: cls.value?.section_name ?? '',
    room: cls.value?.room ?? '',
...
watch(() => form.academic_year_id, (newVal, oldVal) => {
    if (oldVal !== '') {
        form.term_id = '';
    }
});
...
<template>
    <Head :title="`Edit Class: ${cls?.course?.course_code} ${cls?.section_name}`" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Edit Class</h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <Breadcrumb :items="[
                    ...
                    { label: cls?.course?.name ?? 'Class', href: route('admin.classes.show', cls?.id) },
                    { label: 'Edit' },
                ]" />
                ...
                <Card>
                    <PageHeader
                        :title="`Edit Class: ${cls?.course?.course_code} ${cls?.section_name}`"
                        description="Update the details for this class section."
                    />
