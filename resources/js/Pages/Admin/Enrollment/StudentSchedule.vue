                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                                        {{ enrollment.class_course?.room || '—' }}
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                                        <template v-if="enrollment.class_course?.schedule?.length">
                                            <div v-for="(slot, i) in enrollment.class_course.schedule" :key="i" class="whitespace-nowrap">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <Link
                                            :href="route('admin.class-courses.show', enrollment.class_course_id)"
                    ...
                    <div class="block sm:hidden space-y-4">
                        ...
                        >
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Class {{ enrollment.class_course?.class?.class_number ?? '—' }}
                                <template v-if="enrollment.class_course?.room">
                                    &bull; Room: {{ enrollment.class_course.room }}
                                </template>
                            </p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Instructor: {{ enrollment.class_course?.employee?.first_name }} {{ enrollment.class_course?.employee?.last_name }}
                            </p>
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                <template v-if="enrollment.class_course?.schedule?.length">
                                    <div v-for="(slot, i) in enrollment.class_course.schedule" :key="i">
                                        {{ formatScheduleSlot(slot) }}
                                    </div>
                                </template>
