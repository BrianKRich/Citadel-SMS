...
const form = useForm({
    ...
    phone_area_code: props.guardian.phoneNumbers?.[0]?.area_code ?? '',
    phone: props.guardian.phoneNumbers?.[0]?.number ?? '',
    address: props.guardian.address ?? '',
    occupation: props.guardian.occupation ?? '',
    students: props.guardian.students?.map(s => ({ id: s.id, is_primary: s.pivot?.is_primary ?? false })) ?? [],
});
...
function submit() {
    form.patch(route('admin.guardians.update', props.guardian.id));
}
</script>

<template>
    <Head :title="`Edit: ${guardian.first_name} ${guardian.last_name}`" />
        ...
        <div class="py-12">
            <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
                <Breadcrumb :items="[
                    ...
                    { label: `${guardian.first_name} ${guardian.last_name}`, href: route('admin.guardians.show', guardian.id) },
                    { label: 'Edit' },
                ]" />
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <PrimaryButton type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Saving...' : 'Update Guardian' }}
                            </PrimaryButton>
                        </div>
                    </form>
                </Card>
