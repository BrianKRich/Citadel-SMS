...
const props = defineProps({
    classCourses: { type: Object, required: true },
...
function applyFilters() {
    router.get(route('admin.class-courses.index'), {
                ...
                <Card>
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                        <PageHeader
                            ...
                            :description="`${classCourses.total} assignment${classCourses.total === 1 ? '' : 's'} total`"
                        />
                        <div class="sm:ml-auto">
                            <Link
                                :href="route('admin.class-courses.create')"
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-900">
                                <tr v-if="classCourses.data.length === 0">
                                    <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No assignments found.</td>
                                </tr>
                                <tr
                                ...
                                >
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium space-x-4">
                                        <Link :href="route('admin.class-courses.show', cc.id)" class="text-primary-600 dark:text-primary-400 hover:text-primary-900 dark:hover:text-primary-300">View</Link>
                                        <Link :href="route('admin.class-courses.edit', cc.id)" class="text-primary-600 dark:text-primary-400 hover:text-primary-900 dark:hover:text-primary-300">Edit</Link>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="md:hidden space-y-4">
                        <div v-if="classCourses.data.length === 0" class="text-center text-sm text-gray-500 dark:text-gray-400 py-8">No assignments found.</div>
                        <div
                            v-for="cc in classCourses.data"
                            :key="cc.id"
                        ...
                        >
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Class {{ cc.class?.class_number ?? '—' }}</p>
                            ...
                            <div class="flex gap-2">
                                <Link :href="route('admin.class-courses.show', cc.id)" class="flex-1 text-center rounded-md bg-primary-600 px-3 py-2 text-sm font-semibold text-white hover:bg-primary-700 min-h-[44px] flex items-center justify-center">View</Link>
                                <Link :href="route('admin.class-courses.edit', cc.id)" class="flex-1 text-center rounded-md bg-secondary-600 px-3 py-2 text-sm font-semibold text-white hover:bg-secondary-700 min-h-[44px] flex items-center justify-center">Edit</Link>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="classCourses.links && classCourses.links.length > 3" class="mt-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-3 sm:px-6">
                        <div class="flex flex-1 justify-between sm:hidden">
                            <Link v-if="classCourses.prev_page_url" :href="classCourses.prev_page_url" class="relative inline-flex items-center rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50">Previous</Link>
                            <Link v-if="classCourses.next_page_url" :href="classCourses.next_page_url" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50">Next</Link>
                        </div>
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <p class="text-sm text-gray-700 dark:text-gray-300">
                                Showing <span class="font-medium">{{ classCourses.from }}</span> to <span class="font-medium">{{ classCourses.to }}</span> of <span class="font-medium">{{ classCourses.total }}</span> assignments
                            </p>
