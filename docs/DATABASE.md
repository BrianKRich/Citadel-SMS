...
```bash
# PostgreSQL backup
pg_dump citadel_sms > backup_$(date +%Y%m%d).sql
...
# Restore
psql citadel_sms < backup_20260209.sql
```
...
```sql
ALTER DATABASE citadel_sms SET log_min_duration_statement = 1000; -- Log queries > 1 second
```
